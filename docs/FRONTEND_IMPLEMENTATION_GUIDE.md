...
export default withAuth(
  function middleware(req) {
    ...
    if (token && !token.email_verified && !req.nextUrl.pathname.startsWith("/verify-email")) {
...
const loadUserStats = async (userId) => {
  const participants = await fetch(`/api/participants/user/${userId}`).then(r => r.json())
...
async function buildBetContext(market, outcomeId, quantity) {
  const now = new Date()
  const startTime = market.start_time ? new Date(market.start_time) : new Date(market.created_date)
  const closeTime = new Date(market.close_time)
  
  const hoursSinceStart = (now.getTime() - startTime.getTime()) / (1000 * 60 * 60)
  const totalDuration = (closeTime.getTime() - startTime.getTime()) / (1000 * 60 * 60)
  ...
  const dominanceAtEntry = market.participant_count > 0 
...
async function processSettlement(marketId, winningOutcomeId) {
  ...
  const winners = market.participants.filter(p => p.selected_outcome_id === winningOutcomeId)
  ...
  await prisma.$transaction(async (tx) => {
    for (const winner of winners) {
      ...
      const weightedAmount = context?.weighted_amount || winner.amount_contributed
...
*   **Headings:** Bold, `font-bold` (700 weight)
...
import { Home, Users, Wallet, User, Bell, Shield, LogOut, Lock } from 'lucide-react'
import { signOut } from 'next-auth/react'

const navItems = [
  { title: "Home", url: "/", icon: Home, requiresAuth: false },
  { title: "Wallet", url: "/wallet", icon: Wallet, requiresAuth: true },
  { title: "Notifications", url: "/notifications", icon: Bell, requiresAuth: true },
        ...
        <ul className="space-y-1">
          {navItems.map((item) => (
            <li key={item.title}>
              <Link
                href={item.url}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors hover:bg-gray-100 ${currentPath === item.url ? 'bg-gray-100 font-semibold' : ''}`}
              >
          ...
          {isAdmin && (
            <li>
              <Link href="/admin" className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-100">
